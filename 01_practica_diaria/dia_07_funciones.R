...
calcular_porcentaje(15, 60, digitos = 0) # Devuelve 25 (sin decimales)
...
# En bioinformática, el Fold Change (FC) es la razón entre dos medias: media(grupo_A) / media(grupo_B).
# Para evitar la división por cero si una media es 0, se suele sumar un pequeño "pseudocount" (normalmente 1).
#   a) Crea la función 'calcular_fold_change' que acepte dos vectores numéricos: 'grupo_a' y 'grupo_b'.
#   b) Añade el argumento 'pseudocount' con un valor por defecto de 1.
#   c) La función debe calcular las medias de cada grupo (removiendo NAs), sumarles el pseudocount a ambas medias y luego dividirlas (media_a / media_b).
...
#   a) Diseña la función 'filtrar_pvalues' que reciba un vector 'p_vals' y un umbral 'alpha' que por defecto sea 0.05.
#   b) Dentro de la función, comprueba si ALGUN p-value es menor que 0 o mayor que 1. Si es así, detén la ejecución con stop() mostrando un mensaje descriptivo.
#   c) Si todos los valores son válidos, devuelve un vector lógico que sea TRUE para los p-values menores que 'alpha'.
...
#   a) Debe recibir: una matriz de expresión ('matriz') y un factor de diseño con dos grupos ('condiciones').
#   b) Programación defensiva: Comprueba si el número de columnas de la matriz coincide con la longitud del factor 'condiciones'. Si no, lanza un error con stop().
#   c) Utiliza subsetting con corchetes para dividir la matriz en dos submatrices: una para las columnas del Grupo 1 y otra para las del Grupo 2 (según los niveles del factor).
#   d) Usa rowMeans() para calcular el promedio de expresión de cada gen en el Grupo 1 y en el Grupo 2.
#   e) Devuelve una lista con tres elementos: 'media_grupo1', 'media_grupo2' y el cociente de ambas ('fold_change').
